...
import { ApiError, endSession, leaveSession } from "@/lib/api";
...
//
// ホストの「停止」はPOST /sessions/:id/end(全員終了、仕様書§5.6)を伴い、
// ゲストの「停止」(退出)はPOST /sessions/:id/leave(新設)を伴う。
// いずれもこの画面時点ではまだWebSocket接続が無いため、REST経由でサーバーへ
// 明示的に伝える(2026-09-02修正: 以前ゲストの退出はローカルの保存情報を
// 消すだけでサーバーには何も伝えておらず、この画面へたどり着く直前の
// ページ遷移でWebSocketが切れたことによる通常の10分復帰猶予にそのまま
// 乗ってしまい、他の参加者から見て最大10分間参加人数が減らない不具合が
// あった)。ゲストは自分自身の退出のみでき、他の参加者の共有には影響しない
// (既存のホスト/ゲスト権限分離の考え方を踏襲)。
export function ResumeSessionChoice({ stored, onResume, onStopped }: ResumeSessionChoiceProps) {
  ...
  async function doStop(next: "landing" | "create") {
    ...
    setError(null);
    try {
      // 既に失効/終了済み・既に退出済みの場合の404はエラー扱いにしない
      // (結果的にゴールは同じ「もう参加していない」状態のため)。
      try {
        if (isHost) {
          await endSession(stored.sessionId, stored.token, stored.participantId);
        } else {
          await leaveSession(stored.sessionId, stored.token, stored.participantId);
        }
      } catch (e) {
      ...
      setError(e instanceof Error ? e.message : isHost ? "終了に失敗しました。時間をおいて再度お試しください。" : "退出に失敗しました。時間をおいて再度お試しください。");
      setWorking(false);
    }
  }
        ...
        <Card className="w-full max-w-sm">
          <Card.Content className="flex flex-col items-center gap-3 py-8 text-center">
            ...
            <Button variant="primary" fullWidth onPress={() => doStop(next)} isDisabled={working}>
              {working ? "処理中…" : isHost ? (confirming === "stopAndNew" ? "終了して新しく始める" : "終了する") : confirming === "stopAndNew" ? "退出して新しく始める" : "退出する"}
            </Button>
            <Button
              variant="outline"
              fullWidth
      ...
      <Card className="w-full max-w-sm">
        <Card.Content className="flex flex-col gap-3 py-8">
          ...
          <Button variant="outline" fullWidth onPress={() => setConfirming("stopAndNew")}>
            {isHost ? "共有を終了して新しい共有を始める" : "退出して新しい共有を始める"}
          </Button>
          <Button variant="outline" fullWidth onPress={() => setConfirming("stop")}>
            {isHost ? "共有を終了する" : "退出する"}
          </Button>
          {!isHost && <p className="text-xs text-muted">「退出」はあなた自身の参加のみを終了します。他の参加者の共有には影響しません。</p>}
        </Card.Content>
      </Card>
    </div>
  );
